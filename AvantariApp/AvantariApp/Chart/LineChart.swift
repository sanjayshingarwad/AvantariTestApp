...
class LineChart: UIView {
    ...
    let topHorizontalLine: CGFloat = 110.0 / 100
    ...
    private func convertDataEntriesToPoints(entries: [PointEntry]) -> [CGPoint] {
        if let max = entries.max()?.value,
            ...
            for i in 0..<entries.count {
                ...
                let point = CGPoint(x: CGFloat(i)*lineGap + 40, y: height - 20)
                result.append(point)
            }
            return result
        }
    ...
    private func drawHorizontalLines() {
        guard dataEntries != nil else {
            return
        }
        
        let tablePeriods = 10
        let maxvalues:CGFloat = 10
        var gridValues: [CGFloat]? = nil
        
        gridValues = []
        for i in 0...tablePeriods {
            let val = i * ( Int(maxvalues) / tablePeriods)
            gridValues?.append(CGFloat(val))
        }
        
        if let gridValues = gridValues {
            var count = gridValues.count
            for value in gridValues {
                
                let height = (value/maxvalues) * gridLayer.frame.size.height
                let path = UIBezierPath()
                ...
                if (count > 1 && count < tablePeriods+1) {
                ...
                textLayer.string =  "\(gridValues[count-1])"
